...
#     'n_estimators': list(np.arange(950, 1020, 20)),
#     'num_leaves': list(np.arange(40, 70, 10)),
#     'max_depth': list(np.arange(12, 14, 1)),
#     'learning_rate': list(np.arange(0.1, 0.30, 0.02)),
#     'feature_fraction': list(np.arange(0.5, 1, 0.1)),
#     'bagging_fraction': list(np.arange(0.5, 0.8, 0.1))
}
...
# 设置全局字体大小
plt.rcParams.update({'font.size': 20})  # 设置字体大小为20
# 绘制散点图
...
"节省运行时间直接输入SHAP排序结果"
sorted_feature_names=['Qin','CIin', 'RECOVERY', 'CIout', 'HARDout', 'SSout', 'SSin','CODout', 'NH3Nout',
                      'CODin','LEVEL','HARDin','LASin','LASout','NH3Nin','YEAR','CHROMin','CHROMout']

#存储重新选择参数后的参数

best_params_sorted_list=[]
best_mae_sorted_list=[]

rmse_train_sorted_list=[]
mae_train_sorted_list=[]
corr_coef_train_sorted_list=[]
r2_train_sorted_list=[]

rmse_test_sorted_list=[]
mae_test_sorted_list=[]
corr_coef_test_sorted_list=[]
r2_test_sorted_list=[]
...
for i in range(10,len(sorted_feature_names)+1):
    # 读取训练数据和测试数据：
    print("\n",'*'*200)
    ...
    param_grid_sorted = {
        ...
        # 'n_estimators': list(np.arange(960, 1020, 20)),
        # 'num_leaves': list(np.arange(40, 70, 10)),
        # 'max_depth': list(np.arange(12, 15, 1)),
        # 'learning_rate': list(np.arange(0.18, 0.24, 0.02)),
        # 'feature_fraction': list(np.arange(0.7, 1, 0.1)),
        # 'bagging_fraction': list(np.arange(0.5, 0.8, 0.1))
    }

    # 使用GridSearchCV进行参数搜索
    model_sorted = lgb.LGBMRegressor(**params)
    ...
    print("Best Parameters:", best_params_sorted)
    print("Best MAE:", best_mae_sorted)
    best_params_sorted_list.append(best_params_sorted)
    best_mae_sorted_list.append(best_mae_sorted)

    # 训练最佳模型
    best_model_sorted = lgb.LGBMRegressor(**best_params_sorted)
    ...
    rmse_train_sorted_list.append(rmse_train_sorted)
    mae_train_sorted_list.append(mae_train_sorted)
    r2_train_sorted_list.append(r2_train_sorted)
    corr_coef_train_sorted_list.append(corr_coef_train_sorted)
    ...
    rmse_test_sorted_list.append(rmse_test_sorted)
    mae_test_sorted_list.append(mae_test_sorted)
    r2_test_sorted_list.append(r2_test_sorted)
    corr_coef_test_sorted_list.append(corr_coef_test_sorted)
...
print('best_params_sorted_list:',best_params_sorted_list)
print('best_mae_sorted_list:',best_mae_sorted_list)
print('rmse_train_sorted_list:',rmse_train_sorted_list)
print('mae_train_sorted_list:',mae_train_sorted_list)
print('r2_train_sorted_list:',r2_train_sorted_list)
print('corr_coef_train_sorted_list:',corr_coef_train_sorted_list)
print('rmse_test_sorted_list:',rmse_test_sorted_list)
print('mae_test_sorted_list:',mae_test_sorted_list)
print('r2_test_sorted_list:',r2_test_sorted_list)
print('corr_coef_test_sorted_list:',corr_coef_test_sorted_list)
